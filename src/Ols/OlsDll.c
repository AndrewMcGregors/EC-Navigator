	...
	if(gInitDll == FALSE)
	{
		...
		if(gDllStatus == OLS_DLL_NO_ERROR)
		{
			...
			for(int i = 0; i < 4; i++)
			{
				...
				else if(gDllStatus == OLS_DLL_DRIVER_NOT_FOUND)
				       MessageBox(NULL, "DRIVER NOT FOUND", "Initialize",MB_OK);
				else if(gDllStatus == OLS_DLL_DRIVER_NOT_LOADED)
				       MessageBox(NULL, "DRIVER NOT LOADED", "Initialize",MB_OK);
				else   MessageBox(NULL, "Ko ", "Initialize", MB_OK);

				Sleep(100 * i);
			}
		}
		else MessageBox(NULL, "Ko ", "InitDriverInfo ", MB_OK);
...
DWORD Initialize()
{
	...
	{
		MessageBox(NULL, gDriverPath, "Initialization failed!", MB_OK);
...
DWORD InitDriverInfo()
{
	...
	case VER_PLATFORM_WIN32_WINDOWS:
		...
		strcpy(gDriverFileName, OLS_DRIVER_FILE_NAME_WIN_9X);
	...
	case VER_PLATFORM_WIN32_NT:
		...
		strcpy(gDriverFileName, OLS_DRIVER_FILE_NAME_WIN_NT_X64);
 		gDriverType = OLS_DRIVER_TYPE_WIN_NT_X64;

		#else // IA64

		//_tcscpy_s(gDriverFileName, MAX_PATH, OLS_DRIVER_FILE_NAME_WIN_NT_IA64);
		strcpy(gDriverFileName, OLS_DRIVER_FILE_NAME_WIN_NT_IA64);
...

		{
			...
			//tcscpy_s(gDriverFileName, MAX_PATH, OLS_DRIVER_FILE_NAME_WIN_NT_X64);
			strcpy(gDriverFileName, OLS_DRIVER_FILE_NAME_WIN_NT_X64);
			gDriverType = OLS_DRIVER_TYPE_WIN_NT_X64;
...

		{
			...
			strcpy(gDriverFileName, OLS_DRIVER_FILE_NAME_WIN_NT);
...
BOOL IsCpuid()
{
	__try
	{
		int info[4];
		//__cpuid(info, 0x0);
		__cpuid(0x0, info[0], info[1], info[2], info[3]);
	}
...
BOOL IsMsr()
{
	// MSR : Standard Feature Flag EDX, Bit 5
 	int info[4];
	//__cpuid(info, 0x1);
	__cpuid(0x1, info[0], info[1], info[2], info[3]);
	return ((info[3] >> 5) & 1);
}
